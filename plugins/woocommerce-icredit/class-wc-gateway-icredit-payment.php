...
class WC_Gateway_ICredit extends WC_Payment_Gateway_CC{
    ...
    const ICREDIT_PAYMENT_GATEWAY_TOKEN_URL_REAL = 'https://icredit.rivhit.co.il/API/PaymentPageRequest.svc/SaleChargeToken';
...
?>
            <div id="step-payment" class="checkout-step">
                ...
                <div class="checkout-frame" >
                    <iframe id="icredit-iframe" <?php echo $style; ?> width="100%" height="<?php echo $this->iframe_height; ?>" src="<?php echo $icredit_payment_url; ?>" scrolling="yes"></iframe>
                </div><!-- .checkout-frame -->
            </div>
            <?php
        endif;
    ...
    function init_form_fields() {
        ...
        $this->form_fields = array(
			...
			'description' => array(
				...
				'type'        => 'textarea',
				'description' => __('This controls the description which the user sees during checkout.', 'woocommerce_icredit' ),
            ...
            'CreditboxToken' => array(
                'title' => __('Credit box Token - for creating token(optional)', 'woocommerce_icredit'),
                'type' => 'text',
...
function add_payment_method(){
    ...
    $postData =array(
            ...
            'Items' => $items,
            'CustomerFirstName' => $user_first_name,
            'CustomerLastName' => $user_last_name,
            'RedirectURL' => wc_get_page_permalink('myaccount').'payment-methods/',
			//'RedirectURL' => get_site_url().'/index.php/my-account/payment-methods/',
            'CreateToken' => 'true',
            'SaleType' => 3,      
    );
   ...
   return array(
    ...
    function process_payment( $order_id,$url_redirect='' ){
      ...
      $billing_country =  ($order->get_billing_country() == 'IL' || $order->get_billing_country() == '')?'IL':$order->get_billing_country();
        $shipping_country =  ($order->get_shipping_country() == 'IL' || $order->get_shipping_country() == '')?'IL':$order->get_shipping_country();
        ...
        if ( $this->popup_mode == 'iframe' ){
            ...
            $redirect_url = plugins_url( '/', __FILE__ ).'redirect.php?RedirectUrl='.$redirect_url_endcode ;
        }

        $postData['RedirectURL'] = $redirect_url;


        if ($this->field_firstname){ $postData['CustomerFirstName']= $order->get_billing_first_name(); }
        if ($this->field_lastname) { $postData['CustomerLastName']= $order->get_billing_last_name(); }
        if ($this->field_email) {$postData['EmailAddress']= $order->get_billing_email(); }
        if ($this->field_address) {$postData['Address']= $order->get_billing_address_1().' '.$order->get_billing_address_2(); }
        if ($this->field_city) {$postData['City']= $order->get_billing_city();}
        if ($this->field_phonenumber) {$postData['PhoneNumber']= $order->get_billing_phone();}
        if (($order->get_billing_company()) && ($this->field_company)) { $postData['CustomerLastName'].= ' - '.$order->get_billing_company(); }
        if (($order->get_billing_postcode()) && ($this->field_zipcode)) {
            if (!is_numeric($order->get_billing_postcode())) $postData['Zipcode'] = '00000';
            else if (strlen($order->get_billing_postcode()) > 7){
                $postData['Zipcode']= substr($order->get_billing_postcode(),0,7);
            } else {
                $postData['Zipcode']= $order->get_billing_postcode();
            }
        }
        if (($this->user_comments) && ($order->get_customer_note())) {$postData['Comments']= $order->get_customer_note(); }
...
if($_POST['wc-icredit_payment-payment-token'] != NULL && $_POST['wc-icredit_payment-payment-token'] != 'new'){
    ...
    $postData['CreditcardToken'] = WC_Payment_Tokens::get( (int)$token_id ) -> get_token();
...
add_action('woocommerce_review_order_before_submit','remove_form_cc_from_checkout');

function remove_form_cc_from_checkout( ) {
...
add_action('woocommerce_account_add-payment-method_endpoint','remove_form_cc_from_add_payment');   
function remove_form_cc_from_add_payment( ) {
...
add_action('woocommerce_credit_card_form_end','remove_form_cc_from_pay_order');   
function remove_form_cc_from_pay_order( ) {
